...
namespace MessageModel
{
    public class MessageModel{
        ...
        public static async Task<string> RequestReport(DateTime BeginingDate, DateTime EndDate){
            string url = "https://sms.comtele.com.br/api/v2/detailedreporting";
            using (HttpClient httpClient = new()){
                httpClient.DefaultRequestHeaders.Add("auth-key", "7888ffef-72f1-45ce-bd7f-82faf12d7fd6");
                httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));

                string payload = "{\"StartDate\":\"begin_search_data\",\"EndDate\":\"end_search_data\",\"Delivered\":\"filter_status\",\"Receiver\":\"receiver\",\"RequestUniqueId\":\"unique_id\"}";
                HttpContent content = new StringContent(payload, Encoding.UTF8, "application/json");
                using (HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, url)){
                        request.Content = content;
                        HttpResponseMessage response = await httpClient.SendAsync(request);
